    ...
    return ListView(
      children: [
        const Padding(
          ...
          child: Text('Leadership', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ),
        ...senate.leadership.map((official) => _buildOfficialTile(official)),
        const Divider(),
        const Padding(
          padding: EdgeInsets.all(16.0),
          child: Text('Senators', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ),
  ...
  Widget _buildHouseTab(HouseOfRepresentatives house) {
    return ListView(
      children: [
        ...
        const Padding(
          padding: EdgeInsets.all(16.0),
          child: Text('Representatives', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ),
        ...house.representatives.map((official) => _buildOfficialTile(official)),
      ],
    );
  }
  ...
  Widget _buildJudicialTab(GovernmentBranch judicial) {
    return ListView(
      children: [
        ...
        const Divider(),
        if (judicial.supremeCourt != null)
          ...judicial.supremeCourt!.associateJustices
              .map((official) => _buildOfficialTile(official)),
      ],
    );
  }
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          backgroundImage: CachedNetworkImageProvider(official.profilePictureUrl ?? ''),
